...
export const metadata: Metadata = {
  ...
  description:
    "Profiles of young Sierra Leoneans creating measurable impact in their communities.",
  alternates: { canonical: "/young-changemakers" },
  openGraph: {
    title: "Young Changemakers",
    description:
      "Profiles of young Sierra Leoneans creating measurable impact in their communities.",
    type: "website",
    url: "/young-changemakers",
  },
};
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {changemakers.map((c) => (
            <div key={c.slug} className="rounded-2xl border border-navy/10 bg-white p-6 shadow-sm">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-green-ink/10 text-sm font-bold text-green-ink">
                {c.name.split(" ").map((n) => n[0]).join("")}
              </div>
              <h3 className="mt-4 text-sm font-bold text-navy">{c.name}</h3>
              <p className="text-xs font-semibold text-green-ink">{c.initiative} • {c.location}</p>
